...
export const questions: Array<IQuestion> = [
  ...
  const correctId = 0 // correct first answer
  const correctUuid = uuid()

  return {
    ...question,
    uuid: uuid(),
    content: question.content.map((answer, id) => ({
      uuid: id === correctId ? correctUuid : uuid(),
      answer,
    })),
    correct: correctUuid,
  }
...
export function getQuestion(uuids: Array<string>): IQuestion {
  const question = questions[Math.floor(Math.random() * questions.length)]

  if (uuids.includes(question.uuid)) {
    return getQuestion(uuids)
  }
